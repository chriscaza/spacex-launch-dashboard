...
function LaunchCard({ launch, rocketName, launchSite }: Props) {
  return (
    <div className="w-full flex gap-3 bg-white/20 text-white pl-5 py-2 border border-white rounded-lg">
      ...
      <div>
        <p className="text-base font-[var(--font-roboto)] font-semibold mb-1 w-35 truncate">{launch.name}</p>
        <p className="text-base font-[var(--font-roboto)] font-normal mb-1 w-35 truncate">{launchSite}</p>
        <p className="text-base font-[var(--font-roboto)] font-normal mb-1">{rocketName}</p>
        <p className="text-base font-[var(--font-roboto)] font-normal mb-1">{new Date(launch.date_local).toLocaleDateString()}</p>
      </div>
